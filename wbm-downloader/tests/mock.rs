...
/// A manager configuration pointed at `server`, writing under `directory` with the layout the
/// manager tests assert on (`snapshots/` for output, `invalid.db` for the invalid log).
fn manager_configuration(
    server: &MockServer,
    directory: &Path,
    worker_count: usize,
    buffer: usize,
    ...
    ManagerConfiguration {
        output_path: directory.join("snapshots"),
        invalid_log_path: directory.join("invalid.db"),
        client_configuration: configuration(server),
        worker_count,
        buffer,
    }
}
...
/// Queue items for `paths` under `https://example.com/`, each expecting the digest of the body that
/// the path-echoing mock in the manager tests will serve for it.
fn queue_items(paths: &[&str]) -> Vec<ItemInfo<'static>> {
    paths
        .iter()
        .map(|path| {
...
async fn manager_downloads_queue_with_multiple_workers() {
    ...
    let todo = queue_items(&PATHS);

    let manager = Manager::new(
        manager_configuration(&server, directory.path(), 2, PATHS.len()),
        todo.clone(),
    )
    .expect("Cannot build manager");

    let mut results = run_to_completion(manager).await;
...
/// Regression test for a deadlock in `Manager::close`: with the receiver never taken and more items
/// than the result channel can buffer, a worker blocks on a full channel, and `close` must unblock
/// it by closing the channel rather than waiting on it forever.
#[tokio::test]
async fn manager_close_completes_without_receiver() {
    const PATHS: [&str; 4] = ["a", "b", "c", "d"];
    ...
    // A single worker and a one-result buffer guarantee the channel fills before the queue drains,
    // so a `close` that joins workers without closing the channel hangs here.
    let manager = Manager::new(
        manager_configuration(&server, directory.path(), 1, 1),
        queue_items(&PATHS),
    )
    .expect("Cannot build manager");
    ...
    tokio::time::timeout(Duration::from_secs(5), manager.close())
        .await
        .expect("close deadlocked with an undrained receiver")
        .expect("Unexpected manager error");
}

/// A retryable failure that persists past `max_retries` surfaces the final error instead of hanging
/// or succeeding.
#[tokio::test]
async fn download_surfaces_error_when_retries_are_exhausted() {
    let server = MockServer::start().await;
    ...
    assert!(
        matches!(
            result,
            Err(Error::UnexpectedStatus(StatusCode::SERVICE_UNAVAILABLE))
        ),
        "{result:?}"
    );
    // The original attempt plus one request per configured retry.
    assert_eq!(request_count(&server).await, 3);
}

#[tokio::test]
async fn download_fails_on_redirect_without_location() {
    let server = MockServer::start().await;
    ...
    assert!(
        matches!(result, Err(Error::MissingRedirectLocation)),
        "{result:?}"
    );
}

/// Regression test: the follow-up `GET` in `resolve_redirect_shallow` must check the response
/// status, so a transient failure surfaces as an error instead of its error page being returned as
/// content with an apparent digest mismatch.
#[tokio::test]
async fn resolve_redirect_shallow_rejects_failed_content_fetch() {
    let server = MockServer::start().await;

    Mock::given(method("HEAD"))
        .and(path(snapshot_path(URL, TIMESTAMP)))
        .respond_with(redirect_to("https://example.com/moved", "20200101000003"))
        .expect(1)
        .mount(&server)
        .await;
    ...
    // A digest that cannot match the synthesized redirect page forces the content fetch.
    let result = client(&server)
        .resolve_redirect_shallow(URL, timestamp(TIMESTAMP), Sha1Digest::compute(b"mismatch"))
        .await;
...
/// When the synthesized guess does not match the expected digest, the content comes from the
/// redirect response itself (a `302`, like the `HEAD`), and its digest is reported as-is.
#[tokio::test]
async fn resolve_redirect_shallow_fetches_content_when_guess_mismatches() {
    const STORED: &[u8] = b"<html><body>archived redirect page</body></html>";
    ...
    Mock::given(method("GET"))
        .and(path(snapshot_path(URL, TIMESTAMP)))
        .respond_with(
            redirect_to("https://example.com/moved", "20200101000003").set_body_bytes(STORED),
        )
        .expect(1)
        .mount(&server)
        .await;
    ...
    let resolution = client(&server)
        .resolve_redirect_shallow(URL, timestamp(TIMESTAMP), Sha1Digest::compute(STORED))
        .await
        .expect("Unexpected client error");

    assert!(resolution.valid_digest);
    assert_eq!(resolution.content.as_bytes(), STORED);
    assert_eq!(
        resolution.info,
        UrlParts::new("https://example.com/moved", timestamp("20200101000003"))
    );
}

/// A snapshot that fails verification is still written, but under the digest it actually has: the
/// filename must match the downloaded bytes.
#[tokio::test]
async fn manager_writes_mismatched_snapshot_under_actual_digest() {
    let server = MockServer::start().await;
    ...
    let results = run_to_completion(manager).await;

    assert_eq!(
        results,
        vec![DownloadResult::Success {
            url: URL.to_string(),
    ...
    assert!(output_path.join(actual_digest.to_string()).is_file());
    assert!(!output_path.join(expected_digest.to_string()).exists());
}

/// A file already present under the snapshot's content-addressed name is left untouched: the write
/// is skipped rather than repeated.
#[tokio::test]
async fn manager_skips_existing_snapshot_file() {
    const PREEXISTING: &[u8] = b"preexisting bytes";
    ...
    let directory = tempfile::tempdir().expect("Cannot create temporary directory");
    let output_path = directory.path().join("snapshots");
    let digest = Sha1Digest::compute(BODY);
    let snapshot_file = output_path.join(digest.to_string());

    // Sentinel content distinguishable from the download proves the skip: a rewrite would replace
    // it with `BODY`.
    std::fs::create_dir_all(&output_path).expect("Cannot create output directory");
    std::fs::write(&snapshot_file, PREEXISTING).expect("Cannot write existing snapshot file");
    ...
    assert_eq!(
        ...
        vec![DownloadResult::Success {
            url: URL.to_string(),
            timestamp: timestamp(TIMESTAMP),
            expected_digest: Digest::Valid(digest),
            actual_digest: None,
        }]
    );
    assert_eq!(
        std::fs::read(&snapshot_file).expect("Cannot read snapshot file"),
        PREEXISTING
    );
}
