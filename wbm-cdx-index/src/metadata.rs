...
use rocksdb::{BlockBasedOptions, DB, DBCompressionType, IteratorMode, MergeOperands, Options};
use std::path::Path;
...
impl<'a> Iterator for RawEntries<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // `split_first_chunk` splits off a reference to a fixed-size array (like unpacking a
        // struct in Python), which lets the byte conversions below skip runtime length checks.
        let parsed = self
            .bytes
            .split_first_chunk::<8>()
...
impl MetadataDb {
    ...
    pub fn insert(
        ...
        let url_len =
            u16::try_from(url_bytes.len()).map_err(|_| Error::EncodeUrlTooLong(url_bytes.len()))?;

        let mut entry = Vec::with_capacity(ENTRY_HEADER_LEN + url_bytes.len());
        encode_entry(&mut entry, i64::from(timestamp), url_len, url_bytes);

        self.db.merge(digest.0, entry)?;
    ...
    ///
    /// The result has the same length and order as `digests`, with `None` for digests that are
    /// not in the store.
    pub fn multi_get(
        &self,
        digests: &[Sha1Digest],
