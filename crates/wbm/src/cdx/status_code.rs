...
pub enum Error {
    /// The value is not supported by this enumeration.
...
macro_rules! status_codes {
    ($($variant:ident => $code:literal / $str:literal),+ $(,)?) => {
        /// Every [`StatusCode`] variant, in ascending order by value.
        pub const STATUS_CODE_VALUES: [StatusCode; [$($code),+].len()] =
            [$(StatusCode::$variant),+];
        ...
        pub enum StatusCode {
            $(#[doc = concat!("The `", $str, "` status code.")] #[serde(rename = $str)] $variant),+
        }
        ...
        impl StatusCode {
            ...
            /// Look up the status code for an integer value.
            ///
            /// # Errors
            ///
            /// Returns [`Error::Unsupported`] if the value is not supported by this enumeration.
