...
//!
//! The check operation reads a compact Zstandard-compressed NDJSON file and reports, per line,
...
//! consistent state (a timestamp is present, a URL never appears without one, and no field repeats
//! information that serialization would omit).
...
pub struct Summary {
    ...
    /// Digests that are not in strictly ascending order relative to the preceding line.
    pub out_of_order: Vec<Sha1Digest>,
    /// Digests equal to the preceding line's digest.
...
///
/// Reads the Zstandard-compressed NDJSON file at `input` and accumulates a [`Summary`] of schema
...
pub fn check(input: &Path, context: &Context) -> Result<Summary, Error> {
    ...
    for (index, line) in reader.lines().enumerate() {
        ...
        let Ok(snapshot) = crate::exact::ExactSnapshot::parse(&line) else {
