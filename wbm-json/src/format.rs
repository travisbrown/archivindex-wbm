...
//! - [`Codec`] — the `bytes` ↔ `str` pair for a format: a *decode* (raw stored bytes → content
//!   string) and an *encode* (content string + metadata → the exact bytes whose SHA-1 is the
//!   digest). The default [`Format::Utf8`] codec is built into [`Context`](crate::context::Context);
//!   non-default formats register a [`Codec`].
...
impl FormatInfo {
    /// Build a format object with the given name and metadata, and no closing whitespace.
    #[must_use]
    ...
    /// Whether this is the fully-default format object — default [`name`](FormatInfo::name), no
    /// closing whitespace, and no metadata — and so can be omitted from a snapshot.
    #[must_use]
...
/// The `bytes` ↔ `str` codec for a non-default [`Format`].
///
